...
- `arma-reforger-gm01`: a Linux cloud VM running the Arma Reforger dedicated server
- `openclaw-control01`: a separate control host running the director bridge and OpenClaw
...
- `server/arma-reforger-server.env.example`
  Linux environment file for install paths and launch arguments.
- `server/install-update-server.sh`
  Linux SteamCMD update/install script using the official Reforger dedicated-server app id.
- `server/start-reforger-server.sh`
  Linux server launcher that reads the `.env` file and starts `ArmaReforgerServer`.
- `server/arma-reforger-server.service.example`
  Example systemd unit for the Linux game server.
...
- `openclaw/start-openclaw-director.ps1`
  Starts the OpenClaw dashboard or prints the command to run, then renders the current director prompt.
...
- `configure-cloud-bundle.ps1`
  Generates the operator-specific bridge config, game-mode config, reverse-proxy config, and OpenClaw prompt.
...
## Linux game-server quick start

1. Copy `server/arma-reforger-server.env.example` to `server/arma-reforger-server.env` and adjust the install/profile/config paths.
2. Create or place your dedicated-server `config.json` at the path referenced by `AR_SERVER_CONFIG_PATH`.
3. Run:

```bash
chmod +x ./server/install-update-server.sh ./server/start-reforger-server.sh
./server/install-update-server.sh ./server/arma-reforger-server.env
```

4. Install the addons listed in `server/addons-required-dead-everon-bacon.txt`.
5. Ensure the published mission/save includes `OCD_ZombieDirectorGameModeComponent` and the correct `m_sConcreteProfileId`.
6. Upload `server/rest-bridge.profile-override.generated.json` as:
   `$AR_SERVER_PROFILE_DIR/openclaw-zombie-director/rest-bridge.override.json`
7. Make sure the game server can reach `https://director.example.com/`.
8. Start the server with either:

```bash
./server/start-reforger-server.sh ./server/arma-reforger-server.env
```

or the systemd unit in `server/arma-reforger-server.service.example`.
